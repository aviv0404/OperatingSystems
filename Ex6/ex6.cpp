...
private:
    ...
    // A map that links the each FileDescriptor to an index number.
...
public:
    ...
    int CreateFile(string fileName)
    {
        ...
        else
        {
            ...
            MainDir.insert(make_pair(fdNumMain, fd));
    ...
    int OpenFile(string fileName)
    {
        ...
        for (pair<int, FileDescriptor> element : MainDir)
        {
            if (element.second.getFileName().compare(fileName) == 0)
            {
                //loop over all possible fds just in case a file was closed
                for (int i = 0; i <= OpenFileDescriptors.size(); i++)
                {
                    if (OpenFileDescriptors.count(i) == 0)
                    {
                        // assign a new file descriptor number
                        OpenFileDescriptors.insert(make_pair(i, element.second));
                        return i;
                    }
                }
            }
     ...
     * @return -1 if the operation was not successful, and the index it had inside MainDir if it was successful
     */
    int DelFile(string FileName)
    {
        ...
        MainDir.erase(fd);
        OpenFileDescriptors.erase(fd);
        //returns the index inside MainDir, because there is no file descriptor because the file is already close.
        return fd;
    }
