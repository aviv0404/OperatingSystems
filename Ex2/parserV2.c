...
int main() {
    ...
    //main loop
    char* lineNoSpaces = removeSpaces(line);
    while (strcmp(lineNoSpaces, "exit") != 0 && strstr(line,"exit") == NULL) {
        // just spaces or empty string -> skip to the next loop
        if (strcmp(lineNoSpaces, "") == 0){
            printf("Enter String, or \"exit\" to end program:\n");
            fgets(line, MAX_LINE_LENGTH + 1, stdin);
            fflush(stdin);
            line[strcspn(line, "\n")] = '\0';
        ...
        //show history
        if (strcmp(lineNoSpaces, "history") == 0 && strstr(line, "history") != NULL) {
            loadHistory(fileName, &historyList);
 ...
 * char ** historyList - list of strings to save to the file.
 * 
...
*/
void saveHistory(char* fileName, char** historyList) {
